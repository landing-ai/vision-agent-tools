...
class Owlv2Sam2(BaseMLModel):
    ...
    def __init__(self, device: str | None = None):
        """
        Initializes the Owlv2Sam2 object with a pre-trained Owlv2 model
        and a SAM2 model.
        """
        self.device = (
            device
    ...
    def get_bbox_and_mask(
        ...
        confidence: int = 0.3,
    ) -> dict[int, ImageBboxAndMaskLabel] | None:
        objs = {}
        self.image_predictor.set_image(np.array(image, dtype=np.uint8))
        annotation_id = 0
        with torch.autocast(device_type=self.device, dtype=torch.float16):
            preds = self.owlv2(prompt, image, confidence=confidence)[0]
        # TODO: filter out by score
        preds = [asdict(pred) for pred in preds]
        if len(preds) == 0:
            return None
        if return_mask:
    ...
    @torch.inference_mode()
    def handle_image(
        self, prompt: str, image: Image.Image, confidence: int = 0.3
    ) -> dict[int, dict[int, ImageBboxAndMaskLabel]]:
        self.image_predictor.reset_predictor()
        objs = self.get_bbox_and_mask(
            prompt, image.convert("RGB"), confidence=confidence
        )
        return {0: objs}

    @torch.inference_mode()
    def handle_video(
        self, prompt: str, video: VideoNumpy, step: int = 20, confidence: int = 0.3
    ) -> tuple[
    ...
    def __call__(
        ...
        if image is not None:
            return self.handle_image(prompt, image, confidence)
        elif video is not None:
            assert video.ndim == 4, "Video should have 4 dimensions"
            return self.handle_video(prompt, video, step, confidence)
